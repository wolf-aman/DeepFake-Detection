...
async def home(request: Request):
    ...
    return templates.TemplateResponse(
        request=request,
        name="index.html",
        context={"enable_kyc_demo": config.ENABLE_KYC_DEMO},
    )


@app.get("/kyc", response_class=HTMLResponse)
async def kyc(request: Request):
    """Voice KYC workflow interface, disabled unless the feature flag is enabled."""
    if not config.ENABLE_KYC_DEMO:
        raise HTTPException(status_code=404, detail="Voice KYC demo is disabled")
